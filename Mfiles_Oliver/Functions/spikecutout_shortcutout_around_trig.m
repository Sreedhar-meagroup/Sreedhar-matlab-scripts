...
function all_ind=spikecutout_shortcutout_around_trig(ls,CHANNEL_VEC,trig_times,time_window_on, time_window_off)


x_vec       = (-1+0.04):0.04:(2-0.04);
...
for ii=1:nr_ch
    ...
    %the recorded trace, e.g. 0.75 msec or in approx 19 samplesteps
    
    all_pos_spikes = [];
    all_neg_spikes = [];
    
    for jj=1:nr_cutouts
         mean_offset(jj) = mean(ls.context(1:19,all_ind(jj)),1);
         if ls.context(25,all_ind(jj)) - mean_offset(jj) >=0 
