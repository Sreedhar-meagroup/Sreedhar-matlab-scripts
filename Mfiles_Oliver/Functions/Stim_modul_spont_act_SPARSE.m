...
% 
% 
% StimRaster_SPARSE nr_spikes   Return values from StimulusEffect_SPARSE
%                               StimRaster_SPARSE stores the spike times pre and post
...
%
function [Response_burst Prestim_burst Response_stat]= Stim_modul_spont_act_SPARSE(datname,ls,CHANNELS,StimRaster_Sparse,nr_spikes, stim_times,varargin)
...
for ii=1:nr_channels
   ...
   selectedhsub((ii-1)*2+1)              = subplot(subplot_r,subplot_c,plot_pos((ii-1)*2+1-(nr_figs-1)*Nr_ch_per_fig*2)); 
    ...
    for trial=1:TRIALS
            spikes_full = full(StimRaster_Sparse{hw_channels(ii)+1}(:,trial));
            %plotted are of course only he corresp. entries nonzero
            if find(spikes_full)
                plot(spikes_full(find(spikes_full)),trial*ones(1,length(find(spikes_full))),'*k','MarkerSize',2);
                hold on;
            end
...
for ii=1:nr_channels
    for jj=1:TRIALS
        trial_ind     = find(StimRaster_Sparse{hw_channels(ii)+1}(:,jj) >0 );
        %find all the PRE_Spikes in a window of defined length before the
        %stim
        prestim_ind   = find(StimRaster_Sparse{hw_channels(ii)+1}(:,jj) <0 );
        if ~isempty(trial_ind) 
            Responses.time    = [Responses.time full(StimRaster_Sparse{hw_channels(ii)+1}(trial_ind,jj))' + stim_times(jj)];
        ...
        if~isempty(prestim_ind)
             Prestim.time    = [Prestim.time full(StimRaster_Sparse{hw_channels(ii)+1}(prestim_ind,jj))' + stim_times(jj)];
...
for ii=1:nr_channels
    ...
    Response_burst_temp{ii}(1:length(response_burst_ind),5) = num2cell(trial_ind);
        ...
        StimRaster_full = full(StimRaster_Sparse{hw_channels(ii)+1});
        cell_raster{ii}     = mat2cell(StimRaster_full,size(StimRaster_full,1),ones(1,size(StimRaster_full,2)));
...
for ii= 1:nr_channels
    ...
    if ~isempty(spont_act_trials)
        ...
        for kk=1:length(spont_act_trials)
            ...
            if ~isempty(resp_window_spikes)
                resp_abs_spike_times = stim_times(spont_act_trials(kk)) + resp_window_spikes;     %entry no. 3
                ...
                resp_abs_spike_times = stim_times(spont_act_trials(kk));                          %store the stim time in this case
                all_length           = 0;
                spikelength          = 0;
                no_of_spikes         = 0;
                delay                = Inf;
            ...
            Spont_act_trials{ii}{kk,3}     =  resp_abs_spike_times;                        %save the absolute response SPIKE times here        %          
    ...
    if ~isempty(No_resp_ind)
       ...
       elseif isempty(No_resp_ind) & ~isempty(spont_act_trials)
          Response_burst_temp{ii} = [Response_burst_temp{ii};Spont_act_trials{ii}];
       elseif isempty(spont_act_trials) & ~isempty(No_resp_ind)
...
for ii =1:nr_channels
    if ~isempty(Prestim_burst{1,hw_channels(ii)+1})
...
%I can also mark the detected responses in a specific color, on the first
%subplot
for ii=1:nr_channels

    
    axes(selectedhsub((ii-1)*2+1));
    hold on;
    resp_trials  = [Response_burst{ii}{:,5}];
    no_responses = length(resp_trials);
    for kk=1:no_responses
        abs_resp_times = Response_burst{ii}{kk,3};
        rel_resp_times = abs_resp_times - stim_times(resp_trials(kk));
        %if there is a spike at all, and not just the stimulus time
        %calculated here
        if rel_resp_times>0
            plot(rel_resp_times,resp_trials(kk)*ones(1,length(rel_resp_times)),'o','Color',[0 0.5 0],'Markersize',2,'Markerfacecolor',[0 0.5 0],'Markeredgecolor',[0 0.5 0]);
            hold on
        end
    end
end
