...
%21/11/06
%added the conversion for the context data from digital values to analog ones 
%saving the big data files as seperate structures, each eith two columns,
%representing information about "reach" (e.g. 250000) spikes
% .mat files are stored and saved in the working directory with
% 'datname'data_structi.mat
...
raw = fread(fid,[132 reach],'int16');     %fills an 132xinf matrix, where inf actually means until the end of file, i.e. the whole file
...
if ~isnan(range)
  if ~isempty(find([0 1 2 3]==range))
    ...
    y.context(:,iselc) = y.context(:,iselc) .* range_elc/2048;
